...
uint8_t SSD1306_Init(void) {
	...
	if (HAL_I2C_IsDeviceReady(SSD1306_I2C, SSD1306_I2C_ADDR<<1 ,1, 500) != HAL_OK) {
		printf("out of init\n");
		/* Return false */
		return 0;
	}
...
char SSD1306_Putc(char ch, FontDef_t* Font, SSD1306_COLOR_t color) {
    ...
    // Go through font data for the character
    for (i = 0; i < Font->FontHeight; i++) {
        b = Font->data[(ch - 32) * Font->FontHeight + i]; // Get font data for the character
        for (j = 0; j < Font->FontWidth; j++) {
            if ((b << j) & 0x8000) { // Check if the pixel is set
